...
const App: React.FC = () => {
  ...
  const syncWithProjectFile = async (silent = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn('未在代码库根目录找到 vortex_data.json');
      }
    } catch (error) {
  ...
  const handleReset = () => {
    if (window.confirm('警告：确定要重置吗？本地修改将丢失。')) {
    ...
    >
      ...
      <header className={`relative z-[1000] flex justify-between items-center p-6 bg-slate-900/40 backdrop-blur-xl border-b border-white/5 transition-transform duration-500 ${selectedCardId ? '-translate-y-full' : 'translate-y-0'}`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">Vortex 3D Pro</h1>
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{saveStatus === 'saved' ? 'Synced' : 'Syncing'}</span>
            </div>
          </div>
        </div>
        <div className="flex gap-3">
          {mode === AppMode.EDIT && (
            <>
              <button onClick={() => syncWithProjectFile()} disabled={isSyncing} className="flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 transition-all font-bold text-xs"><RefreshCw size={14} className={isSyncing ? 'animate-spin' : ''} /> 同步文件</button>
              <button onClick={handleReset} className="flex items-center gap-2 px-4 py-2 rounded-full bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-400 transition-all font-bold text-xs"><RotateCcw size={14} /> 重置</button>
              <button onClick={handleExportZip} disabled={isExporting} className="flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 transition-all font-bold text-xs">{isExporting ? <Sparkles size={14} className="animate-spin" /> : <Download size={14} />} 打包导出</button>
            </>
          )}
          <button onClick={() => setMode(mode === AppMode.EDIT ? AppMode.PRESENT : AppMode.EDIT)} className={`flex items-center gap-2 px-6 py-2.5 rounded-full transition-all border font-bold text-sm shadow-xl ${mode === AppMode.PRESENT ? 'bg-amber-500/20 border-amber-500/50 text-amber-400' : 'bg-indigo-600/20 border-indigo-500/50 text-indigo-400'}`}>{mode === AppMode.EDIT ? <Presentation size={18} /> : <Edit3 size={18} />} {mode === AppMode.EDIT ? '开启演说' : '返回编辑'}</button>
        </div>
      </header>
      ...
      <main className="flex-1 relative perspective-2000 flex items-center justify-center">
        <div className="relative w-full h-full flex items-center justify-center preserve-3d transition-all duration-300 ease-out" style={{ transform: `scale(${zoom}) rotateX(${rotationX}deg) rotateY(${rotationY}deg)` }}>
          ...
          {cards.map((card, index) => {
            ...
            return (
              <div key={card.id} className="group absolute transition-all duration-[1000ms] cubic-bezier(0.23, 1, 0.32, 1) card-interactive" style={{ transform, opacity, zIndex, width: isSelected ? 'min(1240px, 94vw)' : '320px', height: isSelected ? 'min(840px, 90vh)' : '480px', pointerEvents: isSelected ? 'auto' : (selectedCardId || showOutline ? 'none' : 'auto') }}>
                ...
                {mode === AppMode.EDIT && !selectedCardId && !showOutline && (
                  <button onClick={(e) => removeCard(card.id, e)} className="absolute -top-4 -right-4 p-3 bg-rose-500/20 hover:bg-rose-500 border border-rose-500/50 rounded-full opacity-0 group-hover:opacity-100 transition-all z-[1100] text-white shadow-xl scale-90 hover:scale-110 active:scale-95"><Trash2 size={16} /></button>
                )}
              </div>
            );
          })}
          ...
          {mode === AppMode.EDIT && !selectedCardId && !showOutline && (
            <div className="absolute pointer-events-auto" style={{ transform: 'translateY(120px) translateZ(-50px)' }}>
              <button onClick={addCard} className="flex flex-col items-center gap-4 group">
                <div className="w-16 h-16 rounded-full border-2 border-dashed border-slate-700 flex items-center justify-center bg-slate-900/40 backdrop-blur-md group-hover:border-indigo-500 group-hover:bg-indigo-500/10 transition-all shadow-xl">
                  <Plus size={32} className="text-slate-500 group-hover:text-indigo-400" />
                </div>
                <span className="text-[11px] font-black text-slate-500 group-hover:text-indigo-400 tracking-[0.3em]">ADD SLIDE</span>
              </button>
            </div>
          )}
        </div>
      ...
      <footer className={`relative z-[1000] p-10 text-center transition-all duration-700 ${selectedCardId || showOutline ? 'translate-y-24 opacity-0' : 'translate-y-0 opacity-100'}`}>
        <div className="inline-flex items-center gap-6 px-12 py-5 bg-slate-900/30 backdrop-blur-3xl rounded-full border border-white/5 shadow-2xl">
           <span className="text-slate-500 text-[10px] font-bold tracking-[0.3em] uppercase flex items-center gap-3"><Sparkles size={14} className="text-indigo-500" />左右旋转 · 滚轮缩放 · 向下拖拽大纲</span>
        </div>
      </footer>
    </div>
  );
